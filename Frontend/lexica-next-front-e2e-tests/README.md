...
- Three existing test accounts with equivalent access and configuration.
...
Set `AUTH_EMAIL_USER_C` and `AUTH_PASSWORD_USER_C` for the third account.
...
- `tests/auth/` - Authentication setup for user-a, user-b, and user-c
...
- Three global workers allow the browser projects to run concurrently.
...
Only one invocation may use an account trio at a time, across all machines.
