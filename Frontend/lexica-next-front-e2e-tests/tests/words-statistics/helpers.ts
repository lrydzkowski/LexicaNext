...
export { deleteWordsViaApi } from '../words/helpers';
...
export async function seedOpenQuestionAnswersViaApi(
  ...
  const wordId = await createWordViaApi(page, options.word, options.translation, authToken, createdWordIds);
...
export async function openStatisticsPage(page: Page, query?: Record<string, string>) {
  const url = query ? `/words-statistics?${new URLSearchParams(query).toString()}` : '/words-statistics';
  await page.goto(url);
